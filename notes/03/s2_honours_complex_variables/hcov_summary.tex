...
Let $f_n:S\rightarrow\mathbb{C}$ be continuous functions.
Let $\sum_{j=0}^{\infty}f_j(z)$ converge uniformly on $S$.
Let contour $\Gamma\subset S$. Then:
$$\int_{\Gamma}\sum_{j=0}^{\infty}f_j(z)\dd z
=\sum_{j=0}^{\infty}\int_{\Gamma}f_j(z)\dd z.$$

\subsubsection*{T4.1.23}
Let $D$ be a simply connected domain. Let $f_n\rightarrow f$
uniformly on $D$ and each $f_n$ be holomorphic on $D$.
