...
i.e. that the angle between the image of straight lines
must remain the same.

\subsubsection*{T2.1.2}
Let $U\subseteq\mathbb{C}$ be open and
let $f:U\rightarrow\mathbb{C}$ be a holomorphic function.
Then $f$ preserves angles at every $z_0\in U$ where
$f'(z_0)\neq0$.

i.e. $f$ is a conformal mapping.
...
A matrix $M$ with $\det(M)=1$ and:
$$M\deq\begin{pmatrix}
a & b \\ c & d\end{pmatrix}$$
is associated with M\"obius transformation:
$$f_M(z)=\frac{az+b}{cz+d}$$
with properties $f_{M^{-1}}=f^{-1}_M$ and:
